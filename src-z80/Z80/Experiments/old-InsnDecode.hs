...
z80InsDecode pc z80sys =
  case sysMRead (thePC pc) z80sys of
        (0xdd, z80sys')  -> prefixed 0xdd z80ixTransform z80sys'
        (0xfd, z80sys')  -> prefixed 0xfd z80iyTransform z80sys'
...
-- z: Bits 0, 1, 2
opcComponents :: Z80byte
              -> (Z80byte, Z80byte, Z80byte, Z80byte)
opcComponents opc =
  let z = opc .&. 7
      y = shiftR opc 3 .&. 7
      p = y `shiftR` 1
...
group0decode opc sys pc xForm =
  case opcComponents opc of
    ...
    (1, _, p, 0) -> mkAbsAddrIns sys pc $ LD . Reg16Imm (pairSP reg16XFormF p)
    (1, _, p, 1) -> oneByteInsn (ADD16 $ alu16XFormF $ DestHL (pairSP reg16XFormF p)) sys pc
    (2, _, 0, 0) -> oneByteInsn (LD ToBCindirect) sys pc
    (2, _, 1, 0) -> oneByteInsn (LD ToDEindirect) sys pc
    (2, _, 2, 0) -> mkAbsAddrIns sys pc $ LD16 . indirectStoreXFormF . FromReg16 (RPair16 HL)
    (2, _, 3, 0) -> mkAbsAddrIns sys pc $ LD . AccToMem
    (2, _, 0, 1) -> oneByteInsn (LD FromBCindirect) sys pc
    (2, _, 1, 1) -> oneByteInsn (LD FromDEindirect) sys pc
    (2, _, 2, 1) -> mkAbsAddrIns sys pc $ LD16 . indirectLoadXFormF . ToReg16 (RPair16 HL)
    (2, _, 3, 1) -> mkAbsAddrIns sys pc $ LD . AccFromMem
    (3, _, p, 0) -> oneByteInsn (INC (IncDecReg16 $ pairSP reg16XFormF p)) sys pc
    (3, _, p, 1) -> oneByteInsn (DEC (IncDecReg16 $ pairSP reg16XFormF p)) sys pc
    (4, y, _, _) -> reg8DecodedInsnIXYhl reg8XFormF sys pc y (INC . IncDecReg8)
    (5, y, _, _) -> reg8DecodedInsnIXYhl reg8XFormF sys pc y (DEC . IncDecReg8)
    (6, y, _, _) ->
    ...
    _otherwise    -> wrapDecode (Z80undef $ UndefInsn [opc]) pc sys
    where
...
group1decode opc sys pc xform =
  case opcComponents opc of
    ...
    (z, y, _, _) ->
      ...
      in  (DecodedInsn (1+ newpc') (LD (Reg8Reg8 dstReg srcReg)), sys'')

-- | ALU instruction decode (group 2)
group2decode :: Z80byte
             -> Z80system sysType
...
group2decode opc sys pc xForms =
  let (newpc, alu8operand, sys') = aluReg8 (reg8XForm xForms) sys pc (opc .&. 7)
      insCTor    = case opc `shiftR` 3 .&. 7 of
                     ...
                     7          -> CP
                     _otherwise -> undefined
  in  (DecodedInsn (1+ newpc) (insCTor alu8operand), sys')

-- | Group 3 instructions
group3decode :: Z80byte
             -> Z80system sysType
...
group3decode opc sys pc xForm = case opcComponents opc of
    ...
    (5, _, 1, 1) -> wrapDecode (Z80undef $ UndefInsn [opc]) pc sys
    (5, _, 2, 1) -> let (newpc, (nextOpc, sys')) = sysIncPCAndRead pc sys
                    in  edPrefixDecode nextOpc sys' newpc
    (5, _, 3, 1) -> wrapDecode (Z80undef $ UndefInsn [opc]) pc sys
    ...
    (5, 1, 2, _) -> let (newpc, (nextOpc, sys')) = sysIncPCAndRead pc sys
                    in  edPrefixDecode nextOpc sys' newpc
    (5, _, _, _) -> wrapDecode (Z80undef $ UndefInsn [opc]) pc sys
    (6, 0, _, _) -> aluImm ADD
    (6, 1, _, _) -> aluImm ADC
    (6, 2, _, _) -> aluImm SUB
    (6, 3, _, _) -> aluImm SBC
    ...
    _            -> wrapDecode (Z80undef $ UndefInsn [opc]) pc sys
    where
      reg16XFormF   = reg16XForm xForm
...
edPrefixDecode opc sys pc = case (x, z, y, p, q) of
  ...
  (1, 2, _, _, _) -> invalid
  (1, 3, _, _, 0) -> mkAbsAddrIns sys pc $ LD16 . FromReg16 (pairSP nullReg16XFormF p)
  (1, 3, _, _, 1) -> mkAbsAddrIns sys pc $ LD16 . ToReg16 (pairSP nullReg16XFormF p)
  (1, 3, _, _, _) -> invalid
  ...
  (1, 7, 0, _, _) -> wrapDecode (LD FromAtoI) pc sys
  (1, 7, 1, _, _) -> wrapDecode (LD FromAtoR) pc sys
  (1, 7, 2, _, _) -> wrapDecode (LD FromItoA) pc sys
  (1, 7, 3, _, _) -> wrapDecode (LD FromRtoA) pc sys
  ...
  where
    ...
    invalid         = wrapDecode (Z80undef $ UndefInsn [0xed, opc]) pc sys
...
-- | Convert an 8-bit register index to an ALU operand 'ALUOperand'
aluReg8 :: Z80reg8XForm sysType
        ...
        -> Z80byte
        -- ^ Register code
        -> (Z80PC, Z80operand Z80OpndALU, Z80system sysType)
...
displacementInstruction sys pc ins =
  ...
  in  (DecodedInsn (pc + 2) (ins . mkAbstractAddr $ destAddr), sys')
...
mkAbsAddrIns sys pc ins = z80getAddr sys (pc + 1) & _1 %~ xformAddr
  where
    xformAddr addr = DecodedInsn (pc + 3) $ ins . mkAbstractAddr $ addr
...
type Z80ALU16XForm =    Z80operand Z80OpndALU16
                     -> Z80operand Z80OpndALU16

-- | Indirect stores: LD (addr), [HL|IX|IY]
type Z80IndirectStoreXForm = Z80operand Z80OpndLoad16
                           -> Z80operand Z80OpndLoad16

-- | Indirect loads: LD [HL|IX|IY], (addr)
type Z80IndirectLoadXForm = Z80operand Z80OpndLoad16
                          -> Z80operand Z80OpndLoad16
...
ixXFormALU16 (DestHL reg16) = DestIX reg16
...
iyXFormALU16 (DestHL reg16) = DestIY reg16
