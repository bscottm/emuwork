...
z80disbytes nBytes dstate =
  ...
  in  first (mkByteRange sAddr) $ disasmMReadN (fromIntegral nBytes) dstate
...
z80disasciiz dstate =
  ...
  in  case DVU.elemIndex 0 toSearch of
        Nothing  -> (LineComment "BAD/FAULT Z80 ASCIIZ: no zero terminator found.", dstate & disasmSystem .~ sys')
        Just idx -> (foundStr idx
                    , dstate & disasmSystem .~ sys'
                             & disasmCurAddr +~ (PC . fromIntegral) idx
                    )
...
z80disascii nBytes dstate =
  let sAddr = views disasmCurAddr unPC dstate
  in  first (mkAscii sAddr) $ disasmMReadN (fromIntegral nBytes) dstate
...
z80DefaultPostProcessor disElt@DisasmInsn{} dstate = ((Seq.singleton . internalAddrRef) disElt, labelAddresses disElt dstate)
  where
    ...
    labelAddresses (DisasmInsn _addr _bytes insn _cmnt) dstate'
      ...
      , inrange destAddr && not (views disasmSymbolTable (destAddr `H.member`) dstate')
      = dstate' & disasmSymbolTable %~ H.insert destAddr (mkLabel prefix)
                & disasmLabelNum +~ 1
      | otherwise
      = dstate'
      where
